...
#################################################
#						#
# Code to simulate absolute primogeniture case  #
# Eric Roca Fernandez                           #
...
for simulation in range(total_simulations):
	...
	for period_tt in range(periods):
		...
		'''
		f = open('Absolute.txt', 'a')
	
		b_optimal = []
		g_optimal = []
	
		# Attribute gender to the heir
		inheritors = numpy.random.binomial(1,0.5,counties)
		...
		# Uptade state capacity using the optimal solution
		...
		for i in range(counties):
			if Y[i]<0.01:
                ...
                # Output stats                
		for i in range(counties):
			line = str(simulation+100*int(flag))+';'+str(i+1)+';'+str(period_tt+1)+';'+str(old_Y[i])+';'+str(old_capacity[i])+';'+str(counties)+';'+str(g_optimal[i])+';'+str(b_optimal[i])+';1;'+str(removed[i])+'\n'
			f.write(line)
		f.close()
		...
		line = str(simulation+100*int(flag))+';'+str(period_tt+1)+';1;'+str(len(to_eliminate))+';'+str(marriages)+'\n'
		q = open('external_data_absolute.txt','a')
		q.write(line)
		q.close()
		line = ''
